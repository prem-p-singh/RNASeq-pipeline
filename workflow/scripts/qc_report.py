...
The report title and reference come from --project/--species/--reference, which
the qc_report rule fills from this run's config.
...
<p class="note">{len(recs)} sample(s). Reference {reference} (decoy-aware salmon).
...
def main():
    ...
    ap.add_argument("--reference", default="", help="reference accession")
    ap.add_argument("--assembly", default="", help="reference assembly name")
    a = ap.parse_args()

    sample_ids = read_sample_ids(a.samples)
    name, species, reference, title = identity(
        a.project, a.species, a.reference, a.assembly)
