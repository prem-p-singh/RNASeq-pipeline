---
title: "GO Enrichment of 2021 DE Results"
...
output <- lapply(DE.files, function(filename){
  ...
  outfile <- file.path("GO_results_2021", outfile)
  write.table(out, file = outfile, quote = FALSE, sep = "\t")
  contrast <- gsub("_DE_analysis.txt", "", basename(filename))
  tab <- kable(head(out, 100), caption = paste0("GO term enrichment analysis for ", contrast), row.names = FALSE) %>% kable_styling() %>% scroll_box(height = "500px")
  plot <- dotplot(tmp)
